 ...
 *             添加订阅者类名(全名)到 {@link #EXTRA_SUBSCRIBER_NAME}, 或添加一组订阅者类名(全名)
 *             到 {@link #EXTRA_SUBSCRIBER_NAME_ARRAY} 或 {@link #EXTRA_SUBSCRIBER_NAME_ARRAY}.
...
public abstract class ObservableActivity extends AppCompatActivity {
     ...
     * 订阅者类名(全名)
     */
    public static final String EXTRA_SUBSCRIBER_NAME = "subscriber_name";
    /**
     * 订阅者类名(全名) 数组
     */
    public static final String EXTRA_SUBSCRIBER_NAME_ARRAY = "subscriber_name_array";
    /**
     * 订阅者类名(全名) 集合
     */
    public static final String EXTRA_SUBSCRIBER_NAME_LIST = "subscriber_name_list";
     ...
     * @return 是否成功订阅
     */
    protected boolean subscribe(String classOfSubscriber) {
        try {
            return subscribe((Class<? extends ActivitySubscribe>) Class.forName(classOfSubscriber));
     ...
     * @return 是否成功订阅
     */
    protected boolean subscribe(Class<? extends ActivitySubscribe> classOfSubscriber) {
        try {
            ActivitySubscribe callback = classOfSubscriber.newInstance();
     ...
     * @return 是否成功订阅
     */
    protected boolean subscribe(ActivitySubscribe subscriber) {
        ...
        while (clazz != null) {
            ...
            for (Class<?> callbackClass : interfaces) {
                if (ActivitySubscribe.Register.DATA.contains(callbackClass)) {
     ...
     * @param classOfSubscriber 订阅者类对象
     */
    protected void unsubscribe(Class<? extends ActivitySubscribe> classOfSubscriber) {
        while (classOfSubscriber != null) {
            Class<?>[] interfaces = classOfSubscriber.getInterfaces();
            for (Class<?> callbackClass : interfaces) {
                if (ActivitySubscribe.Register.DATA.contains(callbackClass)) {
                    Set<ActivitySubscribe> list = mSubscribers.get(callbackClass);
                    if (list != null) {
                        Iterator<ActivitySubscribe> it = list.iterator();
                        while (it.hasNext()) {
                            ActivitySubscribe callback = it.next();
                            if (callback.getClass() == classOfSubscriber) {
                                it.remove();
                            }
                        }
                    }
     ...
     * @param subscriber 订阅者
     */
    protected void unsubscribe(ActivitySubscribe subscriber) {
        Class clazz = subscriber.getClass();
        while (clazz != null) {
            Class<?>[] interfaces = clazz.getInterfaces();
            for (Class<?> callbackClass : interfaces) {
    ...
    /*
    正则1:
    ((public|protected) \S+ o([^\(]+)(\(.*\)) *\{\n *super.+;\n *)\}

    $1\nif (mSubscribers != null) {\nSet<ActivitySubscribe> callbacks = mSubscribers.get(ActivitySubscribe.O$3.class);if (callbacks != null) {
    for (ActivitySubscribe callback : callbacks) {\n((ActivitySubscribe.O$3) callback).o$3$4;\n}}}}
    ...
    */
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String subscriber = getIntent().getStringExtra(EXTRA_SUBSCRIBER_NAME);
        if (subscriber != null) {
            subscribe(subscriber);
        }
        String[] subscriberArr = getIntent().getStringArrayExtra(EXTRA_SUBSCRIBER_NAME_ARRAY);
        if (subscriberArr != null && subscriberArr.length > 0) {
            for (String s : subscriberArr) {
                subscribe(s);
            }
        }
        ArrayList<String> subscriberList = getIntent().getStringArrayListExtra(EXTRA_SUBSCRIBER_NAME_LIST);
